...
- If the request requires verifying bug fixes, running system tests, or generating test cases, route it to `qa-tester`.
- `research` is for read-only discovery.
- `self` is only for focused follow-up work in the current context.
- `project-orchestrator` may hand work back and forth between `prompt-engineer`, `os-developer`, and `qa-tester` until the prompt is stable, the task is implemented, and it passes tests.

Keep the orchestration loop simple: refine the prompt, choose the right role, delegate the smallest useful slice of work, run QA to verify, and reconcile the result before moving on.
